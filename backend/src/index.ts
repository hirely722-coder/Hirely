  ...
  try {
    ...
    (async () => {
      try {
        ...
        if (isPdf) {
          // Use Eden AI Resume Parser API — handles scanned, image-based, and custom font PDFs
          const edenApiKey = process.env.EDENAI_API_KEY;
          if (!edenApiKey) throw new Error('EDENAI_API_KEY is not configured.');

          const formData = new FormData();
          formData.append('providers', 'affinda');
          formData.append('language', 'en');
          formData.append('file', new Blob([arrayBuffer], { type: 'application/pdf' }), file.name);
          ...
          if (!response.ok) {
            ...
            console.error('Eden AI Resume Parser failed:', errText);
            throw new Error(`Resume parsing failed (status ${response.status}): ${errText}`);
          }

          const result = (await response.json()) as any;
          console.log('--- EDEN AI RESUME PARSER RAW RESPONSE ---');
          console.log(JSON.stringify(result, null, 2).substring(0, 3000));
          console.log('--- END EDEN AI RESUME PARSER RAW RESPONSE ---');

          // Get the provider result (try affinda first, then any available provider)
          const providerData = result?.affinda || result?.senseloaf || result?.hireability || Object.values(result)?.[0] as any;
          
          if (!providerData || providerData.status === 'fail') {
            const errorMsg = providerData?.error?.message || 'Provider returned no data';
            console.error('Resume parser provider failed:', errorMsg);
            throw new Error(`Resume parser provider error: ${errorMsg}`);
          }

          const extracted = providerData?.extracted_data || providerData;

          // Map Eden AI's standardized resume fields to our schema
          const personalInfos = extracted?.personal_infos || {};
          const name_obj = personalInfos?.name || {};
          const fullName = [name_obj?.first_name, name_obj?.last_name].filter(Boolean).join(' ') || personalInfos?.name?.raw_name || '';
          
          const phones = personalInfos?.phones || personalInfos?.self_summary?.phone_number ? [personalInfos?.self_summary?.phone_number] : [];
          const mails = personalInfos?.mails || [];
          const address_obj = personalInfos?.address || extracted?.address || {};

          const education_entries = extracted?.education?.entries || extracted?.education || [];
          const educationStr = Array.isArray(education_entries) && education_entries.length > 0
            ? education_entries.map((e: any) => [e?.title, e?.establishment].filter(Boolean).join(' at ')).join('; ')
            : '';

          const work_entries = extracted?.work_experience?.entries || extracted?.work_experience || [];
          const currentCompany = Array.isArray(work_entries) && work_entries.length > 0
            ? (work_entries[0]?.company || work_entries[0]?.title || '')
            : '';
          
          // Calculate experience from work entries
          let experienceStr = '';
          if (Array.isArray(work_entries) && work_entries.length > 0) {
            const totalYears = work_entries.reduce((acc: number, w: any) => {
              if (w?.start_date && w?.end_date) {
                const start = new Date(w.start_date).getFullYear();
                const end = new Date(w.end_date).getFullYear();
                return acc + Math.max(0, end - start);
              }
              return acc;
            }, 0);
            experienceStr = totalYears > 0 ? `${totalYears} Years` : `${work_entries.length} role(s)`;
          }

          const skills_list = extracted?.skills || [];
          const skillNames = Array.isArray(skills_list)
            ? skills_list.map((s: any) => (typeof s === 'string' ? s : s?.name || s?.skill || '')).filter(Boolean)
            : [];

          // Build a text summary from all available data
          const summaryParts = [
            fullName ? `Name: ${fullName}` : '',
            educationStr ? `Education: ${educationStr}` : '',
            currentCompany ? `Current/Last Company: ${currentCompany}` : '',
            skillNames.length > 0 ? `Skills: ${skillNames.join(', ')}` : '',
            Array.isArray(work_entries) ? work_entries.map((w: any) => 
              `${w?.title || 'Role'} at ${w?.company || 'Company'} (${w?.start_date || '?'} - ${w?.end_date || 'Present'}): ${w?.description || ''}`
            ).join('\n') : ''
          ].filter(Boolean).join('\n');

          parsedData = {
            name: fullName,
            email: Array.isArray(mails) && mails.length > 0 ? mails[0] : '',
            phone: Array.isArray(phones) && phones.length > 0 ? phones[0] : '',
            skills: skillNames,
            experience: experienceStr,
            education: educationStr,
            currentCompany: currentCompany,
            address: typeof address_obj === 'string' ? address_obj : (address_obj?.formatted || address_obj?.raw_input_location || [address_obj?.city, address_obj?.country].filter(Boolean).join(', ') || ''),
            resumeTextSummary: summaryParts || `${fullName}'s parsed resume.`
          };

          console.log("Mapped parsed data:", JSON.stringify(parsedData, null, 2));

        } else {
          // For .txt files, use the existing LLM approach
...
Return ONLY a valid JSON object matching the requested schema. Do not include any explanation, markdown code blocks, or extra text. Output ONLY the JSON.`;
          ...
          const resumeSchema = {
            ...
            properties: {
              ...
              experience: { type: 'string', description: 'Years or level of experience' },
              ...
              resumeTextSummary: { type: 'string', description: 'Comprehensive summary of the resume' }
            },

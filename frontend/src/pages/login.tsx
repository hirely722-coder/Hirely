...
export default function Login() {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    if (isSignUp) {
      ...
      if (signInErr) {
        setError(signInErr.message || 'Invalid login credentials. Please try again.');
      } else {
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center items-center p-4 relative overflow-hidden text-white font-sans selection:bg-indigo-500/30 bg-[#07070d] w-full">
      ...
      <button
        type="button"
        onClick={() => router.push('/')}
        className="w-full max-w-md mb-4 flex items-center gap-2 text-xs font-semibold text-slate-400 hover:text-white transition-colors z-10 cursor-pointer group"
      >
